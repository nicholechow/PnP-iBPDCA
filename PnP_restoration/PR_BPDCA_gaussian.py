...
image_dir = "../testsets/PrDeep_12"  # 'PrDeep_12' | 'NT6' | 'UNT6'
output_dir = os.path.join('../PR_gaussian', image_dir.split('/')[-1], str(SNR))
...
logger.info(f"choice_scale = {choice_scale}")


def calc_snr(x, xn):
    v_snr = 10 * torch.log10(torch.sum(x**2) / torch.sum((x - xn)**2))
    return v_snr
...
def main(img_dir):
    ...
    def Gaussian_noise_torch(z, SNR):
        z2 = z**2
        signal_power = torch.mean(torch.abs(z2) ** 2)
        snr_linear = 10 ** (SNR / 10)
        noise_power = signal_power / snr_linear
        torch.manual_seed(0)
        intensity_noise = torch.sqrt(noise_power) * torch.randn_like(z2)
    ...
    def At(Ax, SamplingRate=L, mask=Masks):
        ...
        # batch_size * SamplingRate * 128 * 128_complex
        mask_ = torch.conj(mask)
    ...
    YGauss, sigma = Gaussian_noise_torch(torch.abs(A(img_H)), SNR)

    grad_2_f1 = torch.real(torch.fft.ifft2(
        Masks) * torch.fft.fft2(Masks) * torch.abs(A(img_H)**2))
    ...
    grad_2_h = (torch.norm(img_H)**2 + 1) * eyes + 2 * \
        img_H * torch.transpose(img_H, 2, 3)
    ...
    for oit in range(1000):
        ...
        if diff < tol or oit == max_DCA_iter-1:
            ...
            ssim_val = util.calculate_ssim(
                util.tensor2uint(z), util.tensor2uint(img_H))
            fsim_val = fsim(torch.clamp(z, 0, 1).repeat(3, 1, 1).unsqueeze(
                0), img_H.repeat(1, 3, 1, 1), data_range=1.).item()
...
if __name__ == '__main__':
    ...
    for filename in file_list:
        if any(filename.lower().endswith(ext) for ext in supported_extensions):
            ...
            logger.info(
                "---{}--> {} | {:.2f}dB; SSIM: {:.4f}".format(i, filename, psnr_val, ssim_val))
