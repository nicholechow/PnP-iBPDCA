...
output_dir = os.path.join('../PR',image_dir.split('/')[-1], str(SNR))
...
def main(img_dir):
    ...
    L = largest_eigenvalue / smallest_eigenvalue
    L = L.cpu().item()
    delta = 1e-3
    Lambda = np.min([1/(2*delta), 1/L]) *choice_scale
    ...
    def h(z):
        return 0.5 * torch.norm(z)**2 + 0.25 * torch.norm(z)**4
    
    def Dh(x,y):
        return h(x) - h(y) - torch.sum((x-y)*grad_h(y))
    ...
    mu = 1
    backtracking_const = 0.5 # 一步到位
    prev_z = z

    for oit in range(max_DCA_iter):
        zk = z
        beta = (mu - 1) / mu
        mu = 0.5 *(1+ np.sqrt(1+4*mu**2))
        z = zk + beta *(zk - prev_z) #yk
        backtracking_count = 0
        while  Lambda * delta * Dh(zk, prev_z) < Dh(zk, z):
            beta = backtracking_const * beta
            z = zk + beta * (zk - prev_z)
            backtracking_count += 1
        prev_z = zk
        print(f"extrapolation parameter: {beta}, backtracking count: {backtracking_count}")
