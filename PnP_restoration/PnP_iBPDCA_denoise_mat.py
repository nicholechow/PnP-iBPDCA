...
# 5% noise: sigma= 12.75, Lambda= 1.3, gamma * 0.3, max extrapolation: 0.3652
# 3% noise: sigma= 7.65, Lambda= 1.6, gamma * 0.17, max extrapolation: 0.3367
# 1% noise: sigma= 2.55, Lambda= 1.9 gamma * 0.027, max extrapolation: 0.1342

gamma = min((sigma*255)**2, 1/(delta+eta)) * 0.75
...
def main(file_name):
    ...
    for oit in range(max_DCA_iter):
        ...
        diff_list.append(diff.cpu().detach().numpy())
        ...
        if diff < tol or oit == max_DCA_iter-1:
            ...
            fsim_val = fsim(torch.clamp(un,0,1).repeat(3, 1, 1).unsqueeze(0),img_H.repeat(3, 1, 1).unsqueeze(0), data_range=1.).item()
            return psnr_val, ssim_val, fsim_val, oit, diff_list, psnr_list, ssim_list, fsim_list


if __name__ == '__main__':
    beta_name = f'{int(np.round(beta, 2) * 100)}'
    file_name ="t1_ai_msles2_1mm_pn0_rf0_axial_101.jpg"
    psnr_val, ssim_val, fsim_val, oit, diff_list, psnr_list, ssim_list, fsim_list= main(os.path.join(image_dir, file_name))
    print(f"PSNR: {psnr_val}, SSIM: {ssim_val}, FSIM: {fsim_val}, oit: {oit}")

    
    psnr_list_name = 'psnr_' + beta_name
    ssim_list_name = 'ssim_' + beta_name
    fsim_list_name = 'fsim_' + beta_name
    diff_list_name = 'diff_' + beta_name
    data = {
        diff_list_name: np.array(diff_list),
        psnr_list_name: np.array(psnr_list),
        ssim_list_name: np.array(ssim_list),
        fsim_list_name: np.array(fsim_list)
    }
    file_name_without_ext = os.path.splitext(file_name)[0]
    filename = f"run_beta_{beta_name}_sigma_{sigma*255}_{file_name_without_ext}.mat"
