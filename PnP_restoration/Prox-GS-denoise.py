...
output_dir = os.path.join('../results','rician', 'PnP_BPDCA',
...
# tunable parameters
# 10% noise: sigma= 25.5, Lambda= 1.3, 0.4
# 5% noise: sigma= 12.75, Lambda= 1.3, 0.3
# 3% noise: sigma= 7.65, Lambda= 1.6, 0.5
# 1% noise: sigma= 2.55, Lambda= 1.9 0.7
gamma = (sigma**2/2)*0.4
tol = 1e-5
Lambda = 1.3
max_DCA_iter = 1000
...
def main(file_name):
    ...
    return psnr_val, ssim_val, fsim_val


if __name__ == '__main__':
    # psnr_val, ssim_val, fsim_val = main(os.path.join(image_dir, "t1_icbm_normal_1mm_pn0_rf0_sagittal_128.jpg"))
    # print(f"PSNR: {psnr_val}, SSIM: {ssim_val}, FSIM: {fsim_val}")
    psnr_list = []
    ssim_list = []
    fsim_list = []
    ...
    for filename in file_list:
        if any(filename.lower().endswith(ext) for ext in supported_extensions):
            psnr_val, ssim_val, fsim_val = main(
