...
def ricernd(v, s):
    # Check the dimensions of v and s
    if np.isscalar(v):
        dim = np.shape(s)
    elif np.isscalar(s):
        dim = np.shape(v)
    elif np.all(np.equal(np.shape(v), np.shape(s))):
        # Both v and s are non-scalar and have matching dimensions
        dim = np.shape(v)
    else:
        raise ValueError("Sizes of s and v inconsistent.")
